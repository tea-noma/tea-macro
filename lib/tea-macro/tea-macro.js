...
function execute(stmts,context){
	...
	for(var i=0;i<stmts.length;i++){
		...
		case 'include_once':{
			...
			break;}
		case 'include_code':{
			var doc=readFileCodeSync_(exprs[0]);
			if(doc!=null){
				compile(doc, context);
				if(!context.includes){
					context.includes={};
...
function make(basedir, makeobj,target){
	if(target==null){
		for(var k in makeobj){
			if(makeobj[k].outputs){
				makeBlock_(basedir, makeobj[k]);
			}
		}
	}else{
		if(makeobj[target]){
			var obj=makeobj[target];
			if(obj.outputs){
				makeBlock_(basedir, obj);
			}
			if(obj.targets){
				for(var i=0;i<obj.targets.length;i++){
					makeBlock_(basedir, obj.targets[i]);
				}
			}
		}
	}
		...
		if(pats){
			...
			if(o.before){
				if(typeof o.before=='string'){
					tbuf=readFileSync_(basedir+o.before);
					if(tbuf!=null){
						buf+=tbuf;
					}
				}else{
					for(var m=0;m<o.before.length;m++){
						tbuf=readFileSync_(basedir+o.before[m]);
						if(tbuf!=null){
							buf+=tbuf;
						}
					}
			...
			tbuf=readFileSync_(basedir+o.inputs[j]);
			if(tbuf!=null){
				buf+=tbuf;
			}
			if(o.after){
				if(typeof o.after=='string'){
					tbuf=readFileSync_(basedir+o.after);
					if(tbuf!=null){
						buf+=tbuf;
					}
				}else{
					for(var m=0;m<o.after.length;m++){
						tbuf=readFileSync_(basedir+o.after[m]);
						if(tbuf!=null){
							buf+=tbuf;
						}
					}
			...
			var context={params:{},buf:'',mode:'code'};
			compile(buf,context);
			...
			fs.writeFileSync(basedir+output, context.buf+"\n");
		}
	}
}
